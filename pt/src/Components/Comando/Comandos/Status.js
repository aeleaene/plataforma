...
const Status = ({ objetivo }) => {
    ...
    const posicion = (e) => {
        ...
        trackPromise(
            fetch("https://www.protrack.ad105.net/api/devices", requestOptions)
                .then(response => response.json())
                .then((responseData) => {
                    for (var i = 0; i < responseData.length; i++) {
                        if (responseData[i].id === objetivo.id) {
                            console.log(responseData[i].status);
                            if (responseData[i].status === 'online') {
                                setEstado('Status: ' + responseData[i].status + '; NORMAL; GPRS:Link Up; GSM Signal Level:Weak; GPS:ON; ACC:OFF; Defense:OFF;');
                            } else {
                                setEstado('Status: offline NORMAL; GPRS:Link Down; GSM Signal Level:- ; GPS:OFF; ACC:OFF; Defense:OFF;');
                            }
