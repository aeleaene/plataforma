...
const Version = ({ objetivo }) => {
    ...
    const posicion = (e) => {
        ...
        trackPromise(
            fetch("https://www.protrack.ad105.net/api/server", requestOptions)
                .then(response => response.json())
                .then((responseData) => {
                    setReporte('Versión: ' + responseData.version + '.' + responseData.diskUsage);
    ...
    return (
        <s.Content>
            <s.HeadComands><BsChevronDoubleRight /> Versión</s.HeadComands>
            <s.ComandContent>
                <s.InputGroup>
