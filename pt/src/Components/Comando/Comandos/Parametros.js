...
const Parametros = ({objetivo}) => {
                ...
                .then((responseData) => {
                    for (var i = 0; i < responseData.length; i++) {
                        if (responseData[i].id === objetivo.id) {
                                setReporte("IMEI: "+responseData[i].uniqueId+";TIME: "+responseData[i].lastUpdate+"; SENDS: "+responseData[i].positionId+"; SOS: "+responseData[i].contact+";");
                            }
    ...
    return (
        <s.Content>
            <s.HeadComands><BsChevronDoubleRight/> Parametros</s.HeadComands>
            <s.ComandContent>
                <s.InputGroup>
